...
<?php
   if($batches!="error"){
	...
	<tr>
			...
			<th width="30%"><?php echo"Assign to Med. Reviewer"?></th>
	...
	<tr class="<?php echo $batchstatus; ?>">
		...
		<td><?php 
                ...
                <td>
                    <?php echo $this->requestAction(array('controller' => 'Batches','action'=> 'getStatusvalue',$batch['Batch']['status'],'assign.png','Batches','Assign_to_medical_receiver/'.$batch['Batch']['id'],10,$batch['Batch']['id']));
                    //echo $this->Html->link($this->Html->image('assign.png',array('title'=>'assign to Medical receiver')), array('controller'=>'Batches','action' => 'Assign_to_medical_receiver', $batch['Batch']['id']),array('escape'=>FALSE)); ?>
                </td>
                <td>
		...
		<td>
                ...
                <td><?php echo $this->requestAction(array('controller' =>  'Batches','action' => 'getAssignerName/'.$batch['Batch']['id'].'/3')) ?></td>
                <td>
                   <?php
                   echo $this->requestAction(array('controller' => 'Batches', 'action' => 'getStatusvalue', $batch['Batch']['status'], 'assign.png', 'Batches', 'assignBackToClaimsManager/' . $batch['Batch']['id'].'/listbatchesformedicalmanager',1,$batch['Batch']['id']));
...
<div class="actions">
    ...
    <ul>
        <li><?php echo $this->Html->link('List Batches', array('controller'=>'Batches','action' => 'listbatchesformedicalmanager')); ?> </li>
    </ul>
</div>
<?php
